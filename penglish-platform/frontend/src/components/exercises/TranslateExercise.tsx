...
export default function TranslateExercise({ exercise, onComplete }: TranslateExerciseProps) {
  ...
  const handleSubmit = () => {
    ...
    // More lenient matching for translation exercises
    const userNormalized = userAnswer.trim().toLowerCase().replace(/[.,!?]/g, '');
    const correctNormalized = exercise.correctAnswer.toLowerCase().replace(/[.,!?]/g, '');
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      ...
      <motion.div
        ...
        className="mb-8"
      >
        <div className="inline-block px-3 py-1 bg-primary-100 text-primary-700 rounded-lg text-sm font-semibold mb-3">
      ...
      >
        ...
        <p className="mt-2 text-sm text-surface-500">
          Tip: Use Latin letters to write the Farsi pronunciation
        </p>
      </motion.div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-xl font-bold mb-1">
                  {isCorrect ? 'Afareen! (Excellent!)' : 'Almost there!'}
                </h3>
                <p className="text-surface-700 mb-2">
                  {isCorrect
                    ? 'Perfect translation!'
                    : 'Your answer is close, but here is the standard way to write it:'}
                </p>
                {!isCorrect && (
                  <div className="bg-white p-3 rounded-lg">
                    <p className="text-sm text-surface-600 mb-1">Correct answer:</p>
                    <p className="text-lg font-bold text-primary-600">{exercise.correctAnswer}</p>
                  </div>
                )}
              </div>
            </div>
